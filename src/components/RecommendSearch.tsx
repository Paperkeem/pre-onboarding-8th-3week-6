...
import { RecentSerach } from "./RecentSerach";
...
export const RecommendSearch = ({ searchWord, setSearchWord, onFocus }: childProps) => {
   ...
   useEffect(() => {
      document.addEventListener("keydown", (e: KeyboardEvent) => focusListSearch(e, focusItem));
      return () => {
   ...
   const focusContralArrow = (e: KeyLIEvent, index: number, listLength: number) => {
                     ...
                     recommendWord?.map((item, index) => {
                        return (
                           <li
                              ...
                              onKeyDown={(e) => focusContralArrow(e, index, recommendWord?.length)}
